...
class ElasticSearchLocalLogsSender:
    ...
    def send_local_logs_to_elastic(self, elastic_search_connection):
        if self._is_failed_log_file_contain_logs():
            self._write_local_logs_to_elastic(elastic_search_connection)
            self._clean_failed_log_file()

    def _is_failed_log_file_contain_logs(self):
        return os.path.exists(self.path) and os.stat(self.path).st_size > 0

    def _write_local_logs_to_elastic(self, elastic_search_connection):
        bulk_data = []
    ...
    def _clean_failed_log_file(self):
